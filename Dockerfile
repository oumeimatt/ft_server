...
#                                                 +#+#+#+#+#+   +#+            #
#    Created: 2020/11/15 09:20:46 by oel-yous          #+#    #+#              #
#    Updated: 2020/11/18 17:01:27 by oel-yous         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
RUN apt-get install -y php7.3 php7.3-common php7.3-mbstring php7.3-mysql php7.3-fpm php7.3-readline php7.3-opcache php7.3-jso 
...
EXPOSE 80 443
CMD chown -R mysql:mysql /var/lib/mysql /var/run/mysqld && service mysql start && service php7.3-fpm start && service nginx start && echo "CREATE DATABASE wp_db;" | mysql -u root && mysql -u root < /temp/localhost.sql && echo "GRANT ALL PRIVILEGES ON wp_db.* TO 'wp_user'@'localhost' IDENTIFIED BY '123';" | mysql -u root  ; cat
